...
public class CouponController extends PcBaseController {
	...
	public void productCoupons() {
		...
		for(Coupon coupon : list){
			coupon.put("isGeted",0);
		}

		if(customer.getId()!=null){
			for(Coupon Coupon:list){

				Integer customerCoupon  = CouponProductService.listCustCoupon(customer.getId(),Coupon.getInt("id"));
				if(customerCoupon>0){
					Coupon.put("isGeted",1);
				}
			}
		}
	 ...
	 * 查看某个订单的适用优惠券
	 * @param token
	 * @param products [{product_id:1,amount:3,price:8,shop_id:1},...]
	 */
	@Before(CustomerPcAuthInterceptor.class)
	public void orderCoupons() {
		...
		List<Record> records = this.arrayToList(JSON.parseArray(products));
	...
	private List<Record> arrayToList(JSONArray array) {
		List<Record> list = new ArrayList<Record>();
		for (int i = 0; i < array.size(); i++) {
			Record record = new Record();
			JSONObject obj = array.getJSONObject(i);
			record.set("product_id", obj.getInteger("product_id"));
			record.set("amount", obj.getInteger("amount"));
			record.set("price", obj.getDouble("price"));
			record.set("shop_id", obj.getInteger("shop_id"));
			list.add(record);
		}
		return list;
	}
	 ...
	 */
	@Before(CustomerPcAuthInterceptor.class)
	public void listCustomerCoupon(){

		String token = getPara("token");
		Customer customer = (Customer)CacheHelper.get(token);

		List<Record>list = CouponProductService.listCustomerCoupon(customer.getId());

		jsonObject.put("customerCoupon",list);
		renderJson(jsonObject);
	}
	 ...
	 */
	@Before(CustomerPcAuthInterceptor.class)
	public void updateCustomerCouponisUsed(){
		...
		String token = getPara("token");
		Customer customer = (Customer)CacheHelper.get(token);
		Integer couponId = getParaToIntDefault("couponId");

		ServiceCode code = CouponProductService.updateCustomerCoupon(customer.getId(),couponId);
		if (code != ServiceCode.Success) {
