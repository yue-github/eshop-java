 ...
 * @author TangYiFeng
 */
public class CustomerWebAppAuthInterceptor implements Interceptor {
	...
	public void intercept(Invocation inv) {
		 ...
		 if (token != null && !token.equals("")) {
			 ...
			 //主要处理缓存存在，但数据库不存在信息的情况
			 if (ct != null) {
				 ct = Customer.dao.findById(ct.getId());
				 isLogin = (ct != null) ? true : false;
			 }
		 }
		 ...
		 if(!isLogin) {
			 ...
			 if(code == null || code == "") {
				 ...
				 try {
					 ...
					 } else if (customer == null || customer.getDisable() == 1) {
						 return;
					 }
